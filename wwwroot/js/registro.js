document.addEventListener('DOMContentLoaded', () => {
    ...
    // Esc�ner o escritura r�pida - con detecci�n de velocidad
    let folioScanTimer;
    ...
    folioInput.addEventListener('input', () => {
        ...
        folioScanTimer = setTimeout(async () => {
            ...
            if (existe) {
                ...
                folioInput.classList.remove('error');
                if (duration <= 500) reservaInput.focus(); // solo avanza si fue escaneo r�pido
            }
        }, 300);
    });
    ...
    if (numeroOperadorInput) {
        numeroOperadorInput.addEventListener('input', async () => {
            ...
            try {
                ...
                valetNombreInput.value = '';
                valetNombreHidden.value = '';
                console.warn("No se encontr� el operador:", error);
            }
    ...
    (() => {
        ...
        document.addEventListener('keydown', async function (event) {
            ...
            if (event.key === 'Enter') {
                ...
                // Limpiamos cualquier auto-envío pendiente del escaneo: ya
                // estamos resolviendo el Enter aquí, no queremos que el
                // timeout de abajo dispare un segundo envío más tarde.
                ...
                if (buffer.length === 0) {
                    ...
                    // requestSubmit() (no submit()) para que SÍ dispare el
                    // evento 'submit' del form y el guard anti-doble-envío
            ...
            if (/^[0-9a-zA-Z]$/.test(event.key)) {
                ...
                typingTimeout = setTimeout(async () => {
                    ...
                    if (duration <= maxScanDuration && buffer.length >= 7) {
                        ...
                        console.log("Entrada manual detectada o buffer muy corto, NO se env�a autom�ticamente.");
                    }
                    buffer = '';
                }, 7000);
        ...
        async function processBuffer() {
            ...
            if (document.getElementById('ValetNombreHidden').value.trim() !== '') {
                ...
                // requestSubmit() para que el guard anti-doble-envío del
                // listener 'submit' (BotonesView.cshtml) también aplique aquí.
                if (form) form.requestSubmit();
            } else {
                console.warn('Nombre de valet no disponible, no se env�a formulario');
            }
        }
